...
|---|---|---|---|
| 🎯 [Apply Engine](https://github.com/AJ-Protzel/apply-engine) | Aug 2026 | Python, PostgreSQL/Supabase, GitHub Actions, pytest, httpx | A job-application pipeline that sources, filters, and ranks postings from seven public ATS APIs, then stops at the submit button. Ingests into Postgres on a schedule, normalizes and dedupes across sources, and passes every posting through a filter layer of pure functions. Each rejection records the rule that caused it, so an over-strict filter surfaces as a number — on live data, two rules account for three quarters of all kills. Ranking scores role fit against whether the job compounds a résumé, with the model call isolated behind a typed boundary so the queue policy — thresholds, daily and weekly caps, ordering, digest — is deterministic and tested. 164 tests, including seven recorded API payloads replayed offline; writing those found two connectors that had been returning null for a field every geography decision depended on. Archived. |
